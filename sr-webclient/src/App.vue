  ...
  <div v-else class="min-h-screen py-8">
    <div class="mx-auto w-full max-w-[60%]">
      ...
      <div class="mb-8 h-[33vh]">
        <div class="vote-list" ref="voteList">
          <div
            ...
            :key="vote.id"
            class="vote-card animate-vertical-slide-in" :id="index"
            :class="{ 'ring-2 ring-blue-500': selectedVote?.id === vote.id,
          ...
          >
            ...
            <div class="text-sm text-black-500">
              {{ vote.totalCount }} <span v-if="vote.maximumCount !== null"><strong>/ {{ vote.maximumCount }} </strong></span> Total Votes 
            </div>
            <div class="text-sm text-black-500">
              <span v-if="!vote.expiredTime">No end time</span>
              <span v-else-if="Date.now() < Date.parse(vote.expiredTime)">Vote ends at: {{ (new Date(vote.expiredTime).toLocaleString()) }}</span>
      ...
      <div v-if="selectedVote" class="animate-slide-in">
        ...
        <div class="space-y-4">
          <div
            v-for="subject in selectedVote.subjects"
            :key="subject.id"
            class="vote-subject"
          >
            <div class="flex justify-between items-center">
                ...
                <div class="mt-2">
                  <div class="w-full bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      :style="{ width: `${calculatePercentage(subject.voteCount)}%` }"
                    ></div>
                  </div>
              ...
              <button
                v-if="!hasVoted && selectedVote.canVote()"
                @click="castVote(subject.id)"
